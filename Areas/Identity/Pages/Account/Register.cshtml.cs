...
namespace LMS.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        string CreateNewUser( string firstName, string lastName, DateTime DOB, string departmentAbbrev, string role )
        {
            ...
            var allIds = db.Administrators.Select(a => a.UId)
                .Concat(db.Professors.Select(p => p.UId))
                .Concat(db.Students.Select(s => s.UId));

            var maxUid = allIds.OrderByDescending(id => id).FirstOrDefault();
            
            int next = 1;

            if (maxUid != null)
            {
                int current = int.Parse(maxUid.Substring(1));
                next = current + 1;
            }
            //D7 formats into our uID format -> ex: 0000001
            string newUid = "u" + next.ToString("D7");
